  ...
  vi.stubGlobal(
    ...
    vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (url.endsWith("/setup") && method === "POST") {
        return new Response(
          JSON.stringify({ ok: true, config_path: "/tmp/nyctos.toml" }),
          { status: 200, headers: { "content-type": "application/json" } },
        );
      }
      return new Response("{}", {
...
describe("SetupWizard", () => {
  ...
  it("renders the local-llm URL + bearer token fields only when the local-llm runtime is picked, and gates Continue on a non-empty URL", async () => {
    ...
    fireEvent.click(
      screen.getByRole("radio", { name: /Local OpenAI-compatible runtime/i }),
    );
  ...
  it("renders each DoctorCheck shape (passed and failed) on the Sandbox step after Run checks", async () => {
    doctorPayload = {
      checks: [
        ...
        { name: "docker daemon", passed: false, message: "daemon not reachable on /var/run/docker.sock" },
      ],
    };
    renderWizard();
    await screen.findByText("Welcome");
    ...
    expect(
      screen.getByText("daemon not reachable on /var/run/docker.sock"),
    ).toBeInTheDocument();
  });
