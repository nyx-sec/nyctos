...
use nyx_agent_types::product::{RouteModel, RouteModelEndpoint};
...
fn seed_marker(run_id: &str, template_id: &str, route_path: &str) -> String {
    ...
    format!("nyx-agent-{}-{}", template_id.replace('_', "-"), &hash[..8])
}
...
fn seed_object_json(route: &RouteModelEndpoint, marker: &str) -> Value {
    ...
    if obj.is_empty() {
        ...
        obj.insert("nyx_agent_marker".to_string(), Value::String(marker.to_string()));
    }
    Value::Object(obj)
}
...
fn permission_change_json(route: &RouteModelEndpoint, peer_role: &str, marker: &str) -> Value {
    ...
    obj.entry("nyx_agent_marker".to_string()).or_insert_with(|| Value::String(marker.to_string()));
    Value::Object(obj)
}
...
fn webhook_probe_json(route: &RouteModelEndpoint, marker: &str) -> Value {
    ...
    for field in &route.body_fields {
        ...
        let value =
            if lower.contains("signature") || lower.contains("hmac") || lower.contains("sig") {
                ...
                Value::String("nyx-agent.business_logic_probe".to_string())
    ...
    obj.entry("event_type".to_string())
        .or_insert_with(|| Value::String("nyx-agent.business_logic_probe".to_string()));
...
fn password_reset_confirm_json(route: &RouteModelEndpoint, token_var: &str, marker: &str) -> Value {
    ...
    obj.insert("password".to_string(), Value::String(format!("Nyx Agent!{marker}")));
    obj.insert("password_confirmation".to_string(), Value::String(format!("Nyx Agent!{marker}")));
    obj.insert("nyx_agent_marker".to_string(), Value::String(marker.to_string()));
    Value::Object(obj)
}
...
fn refund_json(route: &RouteModelEndpoint, marker: &str) -> Value {
    ...
    obj.insert(
        "reason".to_string(),
        Value::String("nyx-agent authorized replay probe".to_string()),
    );
...
mod tests {
    ...
    use nyx_agent_types::project::{ProjectAuthMode, ProjectAuthProfile};
    ...
    fn options(base: &str) -> LiveVerifierOptions {
        LiveVerifierOptions {
            ...
            auth_artifact_dir: std::env::temp_dir().join("nyx-agent-business-auth-test"),
    ...
    fn tenant_template_defines_roles_seed_captures_and_oracle() {
        ...
        assert!(plan["seed_data"]["object_marker"].as_str().unwrap().starts_with("nyx-agent-"));
