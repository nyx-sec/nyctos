...
use nyx_agent_core::store::PentestCandidateRecord;
...
use nyx_agent_types::payload::{ContextualPayload, PayloadTransport};
use nyx_agent_types::product::{ApiClientCallModel, RouteModel, RouteModelEndpoint};
...
impl<'a> LiveTestPlanSynthesizer<'a> {
    ...
    fn synthesize_avoiding_urls(
        ...
        match strategy {
            ...
            LivePlanStrategy::SsrfUrlFetch => self.no_plan(
                ...
                "SSRF-style URL fetch needs an in-scope callback or seeded local target before Nyx Agent can safely verify it",
            ),
            ...
            LivePlanStrategy::WebhookTrustReviewOnly => self.no_plan(
                candidate,
                NoPlanReasonCode::UnsafeProbe,
                "webhook trust-boundary probes need a seeded harmless event fixture; Nyx Agent will not send synthetic state-changing callbacks by default",
            ),
            ...
            LivePlanStrategy::BusinessLogicReviewOnly => self.no_plan(
                candidate,
                NoPlanReasonCode::UnsafeProbe,
                "credits/payment/business-logic probes are review-only until disposable seeded state is configured; Nyx Agent will not mutate customer or payment data",
            ),
    ...
    fn no_plan_for_missing_capability(
        ...
        ) && matches!(capabilities.seed, EnvCapabilityStatus::Missing)
        {
            return Some(self.no_plan(
                candidate,
                NoPlanReasonCode::SetupMissing,
                "this candidate needs seeded disposable fixture state before Nyx Agent can verify it safely",
            ));
        }
    ...
    fn csrf(
        ...
        self.no_plan(
            ...
            NoPlanReasonCode::MissingSeedData,
            format!(
                "CSRF route {} {} needs seeded form data/reset hooks before Nyx Agent can safely mutate state",
    ...
    fn dom_xss(
        ...
        let payload = "<img src=x onerror=alert('nyx-agent-dom-xss')>";
        let param = candidate_param(candidate).unwrap_or_else(|| "nyx_agent_probe".to_string());
        ...
        LiveTestPlan::BrowserWorkflow(BrowserWorkflowPlan {
            ...
            baseline: Some(Box::new(BrowserWorkflowPlan {
                ...
                oracle: BrowserOracle {
                    alert_contains: vec!["nyx-agent-dom-xss".to_string()],
            ...
            payload: Some(contextual_payload(
                ...
                &param,
                payload,
                "browser dialog contains nyx-agent-dom-xss",
    ...
    fn open_redirect(
        ...
        let redirect_target = "https://nyx-agent.invalid/redirect-probe";
        ...
        request.payload = Some(contextual_payload(
            ...
            &param,
            redirect_target,
            "Location header points to nyx-agent.invalid",
        ...
        LiveTestPlan::SingleHttp(SingleHttpPlan {
            ...
            oracle: HttpOracle {
                ...
                header_contains: BTreeMap::from([("location".to_string(), "nyx-agent.invalid".to_string())]),
                ..HttpOracle::default()
            },
            why_this_confirms: Some(
    ...
    fn cors_misconfiguration(
        ...
        let origin = "https://nyx-agent.invalid";
        let mut request = request_for_endpoint(endpoint, "anonymous");
        ...
        request.payload = Some(contextual_payload(
            ...
            "Origin",
            origin,
            "Access-Control-Allow-Origin reflects or allows nyx-agent.invalid",
    ...
    fn path_traversal(
        ...
        benign.url = append_query(&endpoint.url, param, "nyx-agent-benign.txt");
        LiveTestPlan::SingleHttp(SingleHttpPlan {
            hypothesis: Some(candidate.hypothesis.clone()),
            request,
            baseline: Some(LiveHttpRequest::get(self.ctx.target_urls[0].clone())),
    ...
    fn stateful_lifecycle_recipe(
        ...
        let marker =
            format!("nyx-agent-lifecycle-{}", candidate.id.chars().take(8).collect::<String>());
...
fn member_json(member_role: &str, marker: &str) -> serde_json::Value {
    serde_json::json!({
        "email": format!("{member_role}@nyx-agent.test"),
        "user": member_role,
        "member": member_role,
        "role": "member",
        "marker": marker,
...
mod tests {
    ...
    use nyx_agent_types::product::{RouteEvidence, RouteModel};
    ...
    fn verifier_options(base: &str) -> LiveVerifierOptions {
        LiveVerifierOptions {
            ...
            auth_artifact_dir: std::env::temp_dir().join("nyx-agent-live-planning-auth-test"),
    ...
    fn dummy_test_event_sink() -> nyx_agent_types::event::EventSink {
    ...
    fn auth_profile_with_object(
        ...
        ProjectAuthProfile {
            ...
            mode: nyx_agent_types::project::ProjectAuthMode::HeaderInjection,
            label: None,
            tenant: None,
    ...
    fn endpoint_inference_ignores_local_filesystem_paths() {
        ...
        let candidate = candidate(
            "CONFIG_EXPOSURE",
            "src/auth.rs",
            "Potential configuration exposure: /Users/elipeter/Library/Application Support/nyx-agent/src/auth.rs:46",
        );
    ...
    fn idor_with_configured_owned_object_becomes_authz_object_plan() {
        ...
        let owner_object = ProjectAuthOwnedObject {
            ...
            marker: Some("nyx-agent-owned-project".to_string()),
        };
        let auth = vec![
        ...
        match plan {
            LiveTestPlan::AuthzObjectOwnership(plan) => {
                ...
                assert!(plan
                    ...
                    .positive_markers
                    .iter()
                    .any(|m| m == "nyx-agent-owned-project"));
            }
    ...
    fn idor_uses_creator_member_roles_when_user_a_user_b_are_absent() {
        ...
        creator.bearer_token_env = Some("NYX_AGENT_TEST_CREATOR_TOKEN".to_string());
        let mut member = auth_profile_with_object("member", None);
        member.bearer_token_env = Some("NYX_AGENT_TEST_MEMBER_TOKEN".to_string());
        let auth = vec![creator, member];
        let env = discover_env_capabilities(EnvCapabilityDiscoveryInput {
            target_urls: &targets,
            auth_profiles: &auth,
            auth_env_overrides: &BTreeMap::from([
                ("NYX_AGENT_TEST_CREATOR_TOKEN".to_string(), "creator-token".to_string()),
                ("NYX_AGENT_TEST_MEMBER_TOKEN".to_string(), "member-token".to_string()),
            ]),
            browser_checks_enabled: false,
    ...
    fn capability_report_turns_missing_auth_env_into_setup_missing_no_plan() {
        ...
        user_a.bearer_token_env = Some("NYX_AGENT_TEST_MISSING_USER_A_TOKEN".to_string());
        let mut user_b = auth_profile_with_object("user_b", None);
        user_b.bearer_token_env = Some("NYX_AGENT_TEST_USER_B_TOKEN".to_string());
        let auth = vec![user_a, user_b];
        let env = discover_env_capabilities(EnvCapabilityDiscoveryInput {
            target_urls: &targets,
            auth_profiles: &auth,
            auth_env_overrides: &BTreeMap::from([(
                "NYX_AGENT_TEST_USER_B_TOKEN".to_string(),
