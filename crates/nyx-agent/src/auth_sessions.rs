...
use nyx_agent_types::agent::{AgentTask, Budget, BudgetKind, ExtractedAgentResult};
    ...
    pub async fn acquire_session(
        ...
        if normalized_role == "anonymous" {
            let session = AuthSession {
                ...
                skip_reason: None,
                setup_error: None,
                acquired_at_ms: nyx_agent_core::now_epoch_ms(),
        ...
        let now_ms = nyx_agent_core::now_epoch_ms();
...
fn redacted_otp_source_json(source: &nyx_agent_types::project::ProjectOtpSourceConfig) -> String {
    let value = serde_json::json!({
        "kind": source.kind,
...
fn session_from_playwright_storage(
    ...
    let path = url.path();
    let now_seconds = nyx_agent_core::now_epoch_ms() as f64 / 1000.0;
    let mut pairs = Vec::new();
...
fn session_from_cookie_jar(
    ...
    let now_seconds = nyx_agent_core::now_epoch_ms() / 1000;
    let mut pairs = Vec::new();
    let mut names = BTreeSet::new();
    let mut expires_at_ms: Option<i64> = None;
...
mod tests {
    use super::*;
    use nyx_agent_types::project::{ProjectAuthOwnedObject, ProjectAuthProfile};
    ...
    async fn missing_env_secret_is_explicit_auth_failure() {
        ...
        let profile = ProjectAuthProfile {
            role: "user_a".to_string(),
            bearer_token_env: Some("NYX_AGENT_TEST_MISSING_TOKEN".to_string()),
        ...
        assert!(res.reason.unwrap().contains("NYX_AGENT_TEST_MISSING_TOKEN"));
    }
    ...
    async fn auth_session_uses_runtime_env_overrides() {
        ...
        let profile = ProjectAuthProfile {
            role: "user_a".to_string(),
            bearer_token_env: Some("NYX_AGENT_TEST_USER_A_TOKEN".to_string()),
            ..empty_profile(ProjectAuthMode::HeaderInjection)
        };
        let res = manager
            .acquire_session(
                ...
                &AuthSessionOptions {
                    ...
                    env_overrides: BTreeMap::from([(
                        "NYX_AGENT_TEST_USER_A_TOKEN".to_string(),
    ...
    async fn ai_auto_reports_exact_missing_login_email_env() {
        let profile = ProjectAuthProfile {
            ...
            login_email_env: Some("NYX_AGENT_TEST_MISSING_MEMBER_EMAIL".to_string()),
            password_env: Some("NYX_AGENT_TEST_MEMBER_PASSWORD".to_string()),
            ..empty_profile(ProjectAuthMode::AiAuto)
        };
        ...
        let res = AuthSessionManager::default()
            .acquire_session(
                ...
                &AuthSessionOptions {
                    ...
                    workspace_paths: Vec::new(),
                    env_overrides: BTreeMap::from([(
                        "NYX_AGENT_TEST_MEMBER_PASSWORD".to_string(),
        ...
        assert!(reason.contains("NYX_AGENT_TEST_MISSING_MEMBER_EMAIL"));
        assert!(reason.contains("member"));
        let setup = res.evidence.get("setup_error").expect("setup error");
        assert_eq!(setup["code"], "setup_missing");
        assert_eq!(setup["missing_env_vars"][0], "NYX_AGENT_TEST_MISSING_MEMBER_EMAIL");
