...
use nyx_agent_core::store::{finding_id_hash, PentestCandidateRecord, Store};
...
fn add_business_logic_abuse_lead(drafts: &mut PlaybookDrafts, surface: &EndpointSurface) {
    ...
    drafts.add(PlaybookLead {
        ...
        hypothesis: format!(
            "{} {} touches credits/payment/pricing semantics. This is an unverified business-logic lead; Nyx Agent must not mutate customer/payment state for validation.",
            surface.method, surface.path
        ),
    });
}
...
mod tests {
    ...
    use nyx_agent_types::live_plan::{LiveTestPlan, NoPlanReasonCode};
    use nyx_agent_types::product::RouteEvidence;
