...
impl PayloadRunner {
    ...
    pub async fn verify(&self, run: PayloadRun) -> Result<VerifyResult, PayloadRunnerError> {
        ...
        if matches!(run.harness_source, HarnessSource::Synthesised) && !run.spec.invoke.contains(PAYLOAD_SLOT) {
        ...
        let vuln_run = self
            .single_run(&run, lang, &run.vuln_payload, "vuln")
            .await?;
        let benign_run = self
            .single_run(&run, lang, &run.benign_payload, "benign")
            .await?;
        ...
        if self.replay_stable_check && result.verdict != VerifyVerdict::Errored {
            ...
            let vuln_replay = self
                .single_run(&run, lang, &run.vuln_payload, "vuln-replay")
                .await?;
            let benign_replay = self
                .single_run(&run, lang, &run.benign_payload, "benign-replay")
                .await?;
    ...
    async fn run_sandbox(
        &self,
        opts: SandboxOpts,
    ) -> Result<crate::SandboxOutcome, SandboxError> {
        match self.backend {
            BackendKind::Process => {
...
fn render_synthesised(spec: &HarnessSpecInput, lang: HarnessLang, payload: &[u8]) -> Vec<u8> {
    ...
    match lang {
        HarnessLang::Python => out.push_str("# auto-generated by nyx-agent-sandbox::payload_runner\n"),
        HarnessLang::Shell => out.push_str("#!/bin/sh\n# auto-generated by nyx-agent-sandbox::payload_runner\nset -u\n"),
    }
