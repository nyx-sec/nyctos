...
//!
//!    Strict mode: setting `NYX_AGENT_REQUIRE_DOCKER=1` flips every
...
use nyx_agent_sandbox::env::{EnvBuilder, EnvError, PullPolicy, RepoInput, SecretsError};
...
fn require_docker() -> bool {
    std::env::var("NYX_AGENT_REQUIRE_DOCKER").ok().as_deref() == Some("1")
}
...
async fn two_service_compose_spins_up() {
    ...
    let Some(docker) = docker_available() else {
        if strict {
            panic!("NYX_AGENT_REQUIRE_DOCKER=1 but `docker` is not on PATH");
        }
    ...
    if !docker_daemon_reachable(&docker).await {
        if strict {
            panic!(
                "NYX_AGENT_REQUIRE_DOCKER=1 but docker daemon unreachable (`docker info` failed)"
            );
        }
    ...
    let env = match builder.up().await {
        ...
        Err(err) => {
            ...
            // A strict-mode CI lane sets NYX_AGENT_REQUIRE_DOCKER=1 and
            // gets the real assertion: a bad compose merge / missing
            // flag / regressed secrets check then surfaces as red.
            if strict {
                panic!("NYX_AGENT_REQUIRE_DOCKER=1 but `docker compose up` failed: {err}");
            }
