...
//! the development workflow, and emits a Nyctos launch profile. It is
...
fn build_agent_task(scope: &ProjectSetupScope) -> AgentTask {
    ...
    let system = "You are Nyctos's project setup agent.\n\
        You inspect only user-owned local repositories and prepare a launch profile that Nyctos can run later. \
    ...
    let objective = format!(
        "Project: {project_name} ({project_id})\n\
         ...
         3. Produce a Nyctos launch profile with `mode`, `target_urls`, and command arrays named `build_steps`, `start_steps`, `seed_steps`, `reset_steps`, `login_steps`, `stop_steps`, and `health_checks`.\n\
