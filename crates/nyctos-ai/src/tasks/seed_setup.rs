...
use nyctos_types::product::{ProjectLaunchProfile, SeedSetupPlan};
...
fn build_agent_task(scope: &SeedSetupScope) -> AgentTask {
    ...
    let system = "You are Nyctos's seed setup agent.\n\
        ...
        Do not use production services or raw secrets. Use stable markers such as `nyctos-owned-by-user-a` so later verification can prove object boundaries."
        .to_string();
    ...
    let objective = format!(
        "Project: {project_name} ({project_id})\n\
         ...
         2. Prefer creating or reusing a local app-native seed command such as `npm run nyctos:seed`, `rails db:seed`, `python manage.py seed`, or a repo-provided test fixture command. If a local command requires confirmation, include `stdin` like `\"y\\n\"` on the step.\n\
         ...
         \n\
         Example:\n\
         {{\"tool\":\"record_seed_setup\",\"input\":{{\"plan\":{{\"summary\":\"Prepared deterministic users and one project owned by user_a.\",\"seed_steps\":[{{\"command\":\"npm run nyctos:seed\",\"timeout_seconds\":120}}],\"reset_steps\":[{{\"command\":\"npm run dev:reset\",\"timeout_seconds\":120,\"stdin\":\"y\\n\"}}],\"env_vars\":[{{\"name\":\"NYCTOS_USER_A_USERNAME\",\"value\":\"user-a@example.test\",\"secret\":false}},{{\"name\":\"NYCTOS_USER_A_PASSWORD\",\"value\":\"nyctos-user-a-pass\",\"secret\":true}}],\"roles\":[\"user_a\",\"user_b\"],\"seeded_objects\":[{{\"name\":\"project\",\"id\":\"nyctos-project-a\",\"route\":\"/api/projects/{{id}}\",\"marker\":\"nyctos-owned-by-user-a\"}}],\"checks\":[\"seed script found in package.json\"],\"warnings\":[]}}}}}}\n",
...
fn clean_steps(
    steps: Vec<nyctos_types::product::LaunchStep>,
) -> Vec<nyctos_types::product::LaunchStep> {
    steps
        .into_iter()
        .filter_map(|mut step| {
