...
use nyctos_core::store::{FindingRecord, ProjectIntegrationRecord, ProjectIntegrationStoredRecord};
use nyctos_core::{now_epoch_ms, Store};
use nyctos_types::event::{AgentEvent, RunEvent, SandboxEvent};
...
impl IntegrationDispatcher {
    ...
    pub async fn send_test(
        ...
        let payload = IntegrationDeliveryPayload {
            ...
            run_id: None,
            finding_id: None,
            title: "Nyctos test notification".to_string(),
        ...
        match ev {
            AgentEvent::Run {
                ...
                let title = if vulnerabilities.is_empty() {
                    format!("Nyctos run {run_id} finished")
                } else {
                    format!("Nyctos run {run_id} found {} verified issue(s)", vulnerabilities.len())
                };
                let payload = IntegrationDeliveryPayload {
        ...
        match cfg {
            ProjectIntegrationConfigInput::Webhook { url, signing_secret } => {
                ...
                if let Some(secret) = signing_secret.filter(|s| !s.is_empty()) {
                    ...
                    req = req.header("X-Nyctos-Signature-256", sig);
                }
