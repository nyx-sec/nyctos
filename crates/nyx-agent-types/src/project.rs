...
//! pulling all of `nyx-agent-core` into their dep graph. `nyx-agent-core`
//! re-exports both under `nyx_agent_core::project::{Project, ProjectId}`
...
/// produce the outer `None`. Lifted into `nyx-agent-types` so both
