...
pub use nyx_agent_types::project::{ProjectRecord, ProjectRuntimeProfile};
...
) -> Result<Option<nyx_agent_types::product::ProjectLaunchProfile>, StoreError> {
    let id: Option<String> = row.try_get("lp_id")?;
    let Some(id) = id else {
        return Ok(None);
    };
    Ok(Some(nyx_agent_types::product::ProjectLaunchProfile {
        id,
